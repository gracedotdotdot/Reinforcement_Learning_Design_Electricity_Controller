...
reward=zeros(n_train,1);
final_reward = zeros(n_train,1);
A_array= zeros(n_train,1);
final_price = zeros(n_train,1);
mu = zeros(n_train,1);
...
figure()
t=1:length(energy);
plot(t, energy(:,1),'-o','DisplayName', 'Non-RES power supply','LineWidth', 2);
hold on;
plot(t, energy(:,2),'--','DisplayName', 'Power demand','LineWidth', 2);
hold on;
plot(t, energy(:,3), ':', 'DisplayName', 'Energy storage','LineWidth', 2);
hold on;
plot(t, omg(1:length(energy)),'DisplayName', 'RES power supply','LineWidth', 2);
hold on;
plot(t, omg(1:length(energy))+energy(:,1), 'DisplayName', 'RES and non-RES power','LineWidth', 2);

xlabel('time(15 min)');
ylabel('power(GW) and energy(0.25GWh)');
axis([0 5730 -10 70])
